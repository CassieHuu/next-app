    ...
    <>
      <section className="h-full w-full pt-44 relative flex items-center justify-center flex-col">
        ...
        <div className="flex justify-center items-center relative md:mt-[-70px]">
          ...
          <div className="bottom-0 top-[50%] bg-gradient-to-r dark:from-background left-0 right-0 absolute z-10"/>
        </div>
      </section>
      <section className="flex justify-center items-center flex-col gap-4 md:!mt-20 mt-[-60px]">
        <h2 className="text-4xl text-center">Choose what fits you right</h2>
        <p className="text-muted-foreground text-center">
          Our straightforword pricing plans are tailored to meet your needs. If
          {"you're"} not <br/>
          ...
          {pricingCards.map((card)=> (
            <Card
              ...
              <CardFooter className="flex flex-col items-center gap-4">
                ...
                <Link href={`/agency?plan=${card.priceId}`} className={clsx('w-full text-center bg-primary p-2 rounded-md', {'!bg-muted-foreground': card.title !== 'Unlimited Saas'})}> Get Started</Link>
              </CardFooter>
            </Card>
          ))}
        </div>
